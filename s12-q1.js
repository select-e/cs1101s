...
//        display_environment(E);
        ...
        if (is_literal(command)) {
            ...
            const new_command = bring_functions_to_front(block_body(command));
            const locals = scan_out_declarations(
                             block_body(new_command));
            const unassigneds = list_of_unassigned(locals);
            C = pair(block_body(new_command),
...
function bring_functions_to_front(component) {
    display("BRING");
    // display_list(component);
    if (is_sequence(component)) {
        const functions = filter(is_function_declaration,
                             (sequence_statements(component)));
        set_head(tail(component), append(functions,
                      remove_all(functions, 
                                 sequence_statements(component))));
        display_list(component);
    }
    return(component);
